...
<head>
  ...
  <title>Flightdeck Session Report — ${escapeHtml(data.projectName)}</title>
  <style>
...
<body>
  <h1>🤖 Flightdeck Session Report</h1>
  <div class="meta">
  ...
  <div class="footer">Generated by Flightdeck · ${new Date().toISOString()}</div>
</body>
</html>`;
  }
  ...
  generateMarkdown(data: ReportData): string {
    ...
    let md = `# Flightdeck Session Report — ${data.projectName}\n\n`;
