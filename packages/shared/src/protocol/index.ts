...
export interface AgentCrashedEvent { type: 'agent:crashed'; agentId: string; code?: number }
export interface AgentAutoRestartedEvent { type: 'agent:auto_restarted'; agentId: string; crashCount?: number }
export interface AgentRestartLimitEvent { type: 'agent:restart_limit'; agentId: string }
export interface AgentStatusEvent { type: 'agent:status'; agentId: string; status: string }
...
export interface AgentMessageSentEvent { type: 'agent:message_sent'; from: string; to: string; fromRole?: string; toRole?: string; content: string }
export interface AgentContextCompactedEvent { type: 'agent:context_compacted'; agentId: string; previousUsed?: number; currentUsed?: number; percentDrop?: number }
export interface AgentDelegatedEvent { type: 'agent:delegated'; parentId: string; childId: string; delegation?: Record<string, unknown> }
export interface AgentCompletionReportedEvent { type: 'agent:completion_reported'; childId: string; parentId?: string; status: string }
export interface AgentUsageEvent { type: 'agent:usage'; agentId: string; inputTokens: number; outputTokens: number; cacheReadTokens?: number; cacheWriteTokens?: number; costUsd?: number; contextWindowUsed?: number; contextWindowSize?: number }
export interface AgentSpawnErrorEvent { type: 'agent:spawn_error'; agentId: string; message: string }
export interface AgentModelFallbackEvent { type: 'agent:model_fallback'; agentId: string; agentRole: string; requested: string; resolved: string; reason: string; provider: string }
export interface AgentSessionResumeFailedEvent { type: 'agent:session_resume_failed'; agentId: string; requestedSessionId?: string; error: string }
export interface DagUpdatedEvent { type: 'dag:updated'; leadId: string }
export interface LockAcquiredEvent { type: 'lock:acquired'; agentId: string; filePath?: string; agentRole?: string; reason?: string; lock?: FileLock }
...
export interface GroupCreatedEvent { type: 'group:created'; name: string; leadId: string; memberIds?: string[]; createdAt?: string; projectId?: string }
export interface GroupMessageEvent { type: 'group:message'; leadId?: string; message: Record<string, unknown>; recipientIds?: string[] }
...
export interface LeadProgressEvent { type: 'lead:progress'; leadId?: string; agentId?: string; summary?: string; percent?: number; status?: string }
export interface TimerCreatedEvent { type: 'timer:created'; timer: { id: string; agentId: string; label: string } }
export interface TimerFiredEvent { type: 'timer:fired'; timerId?: string; timer?: { id: string; agentId: string; label: string; message?: string } }
export interface TimerCancelledEvent { type: 'timer:cancelled'; timerId?: string; timer?: { id: string; agentId: string; label: string } }
...
export interface AttentionChangedEvent { type: 'attention:changed'; trigger: string; leadId?: string }
...
export type WsServerMessage =
  ...
  | AgentUsageEvent | AgentSpawnErrorEvent | AgentModelFallbackEvent
  | AgentSessionResumeFailedEvent | AttentionChangedEvent;
