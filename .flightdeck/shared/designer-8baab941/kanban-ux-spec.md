...
**Date:** 2026-03-08  
**Status:** Final — Lead-approved, incorporating PM + Radical Thinker input
...
12. [Converged Design Decisions](#12-converged-design-decisions-designer--pm-agreement)
13. [Incorporations from Radical Thinker Debate](#13-incorporations-from-radical-thinker-debate)
...
## 13. Incorporations from Radical Thinker Debate

After reviewing the Radical Thinker's rethink doc (`.flightdeck/shared/radical-thinker-1390cfd8/dashboard-rethink.md`), the following ideas are incorporated into the design. See the kanban-ux group chat for the full debate.

### What We're Incorporating

#### I1: Attention Bar Escalation States ⭐ Phase 1
The attention strip is not a static counter — it has three visual states that **adapt to the situation**:

- 🟢 **Green Mode**: Strip is minimal/collapsed. "All healthy — 12/20 done." Calm colors, thin bar.
- 🟡 **Yellow Mode**: Strip expands slightly. "1 task stale, 1 decision pending." Amber accent border.
- 🔴 **Red Mode**: Strip is prominent with pulse animation. "2 tasks failed, 1 blocked >30m." Red accent, **auto-expands Command Center**.

**Trigger rules:**
- Green: 0 exceptions (no failed, no blocked >threshold, no stale, no pending decisions)
- Yellow: 1-2 exceptions
- Red: 3+ exceptions OR any failed task

This incorporates both the Radical Thinker's escalation philosophy AND the PM's auto-adaptation idea. The UI RESPONDS to the situation rather than requiring the user to seek out problems.

#### I2: Catch-Up Summary → Command Center Feature ⭐ Phase 2a
When the user returns after >10 minutes of inactivity, the Command Center expanded view opens automatically with a synthesized summary:

```
⏱ While you were away (47 min):
• 8 tasks completed across 2 projects
• 1 decision made (auth approach — JWT, auto-approved)
• 1 decision pending YOUR input (database naming)
• Total cost: $1.24
• Current: 3 agents active, 2 idle
[Jump to Pending Decision] [View All Activity] [Dismiss]
```

This replaces the "return to a static board" experience with a proactive briefing. The catch-up summary is generated by diffing the current state against the last-seen state (stored in localStorage).

#### I3: Trust Dial → "Monitoring Level" Setting 🟢 Phase 3
A project-level setting in Settings that controls notification verbosity and card density:

| Level | Notifications | Card Density | Stale Threshold | Auto-Expand |
|---|---|---|---|---|
| **Detailed** | All status changes | Standard (~80px) | 10 min | Yellow at 1 exception |
| **Standard** (default) | Exceptions only | Standard (~80px) | 15 min | Yellow at 2, Red at 3 |
| **Minimal** | Critical only (Red) | Compact (~36px) | 30 min | Red at 3+ only |

This maps to the PM's card density modes AND the Radical Thinker's Trust Dial concept. Start as a simple 3-option setting; evolve into adaptive detection in future releases.

#### I4: System-Wide Attention Bar 🟢 Phase 3
Move the attention bar from the Kanban page into the **app shell** (Layout component). It's visible on EVERY page — Settings, Knowledge, Analytics, etc. This ensures the user never misses a decision or failure regardless of which page they're on.

**Spec:**
- Position: Top of viewport, below the app title bar
- Height: 28px (Green), 36px (Yellow), 48px (Red)
- Content: Decision count + failed count + cost + runtime
- Click any item → navigate to the relevant task/decision
- Dismissible for the current session (re-appears if new exceptions arrive)

### What We're NOT Incorporating

| Radical Thinker Proposal | Decision | Rationale |
|---|---|---|
| Delete Kanban, use DAG only | ❌ Rejected | DAG already exists as view option (DagGraph.tsx). Kanban excels at status distribution (pre-attentive column shape). DAG excels at dependency understanding. Complementary, not competing. DAG gets chaotic past ~20 nodes; Kanban scales better with scrollable columns. |
| Zoomable single-surface UI | ❌ Rejected | Swiss Army knife risk — one tool doing everything adequately vs. specialized views doing each thing well. Engineering cost (~2500 LOC new) for marginal UX gain over current multi-view approach. |
| Ambient rings/orbs for portfolio | ❌ Rejected | Too abstract for onboarding (Casey persona). Progress bars + text are more universally readable. Rings require visual literacy that not all users have. |
| Delete HomeDashboard | ❌ Rejected | HomeDashboard and Kanban serve different scopes — project health (dashboard) vs task status (kanban). The Command Center mode bridges them for the operational use case without deleting either. |
| Replace toasts with escalation cards | ⏸ Deferred | Good idea but scope too large. Toast system works. Could revisit in a future redesign pass. |

### Updated Phase Roadmap (Final)

**Phase 1 — Immediate:**
- R1: Attention summary strip WITH escalation states (Green/Yellow/Red) ⭐ NEW
- R2: Agent assignment on card face (role + 4-char ID)
- R3: Time-in-status display
- R4: Filter bar (role, agent, priority, text search)
- R5: Better empty state with onboarding CTA
- R6: Conditional auto-collapse Done
- R7: Stale task indicator
- R8: Color semantics fix (done→emerald, strengthen failed red)
- Failed column never hidden by "Hide empty columns"
- Column header tooltips
- Persistent view state (URL + localStorage)

**Phase 2a — Core Interactivity:**
- Context menu (Retry, Reassign, Pause, View Logs, Change Priority)
- Within-column DnD (reorder = priority change)
- Scope switcher (global vs project-specific)
- Team filter chip
- Compact card mode toggle
- "Show recent 5" for Done column
- `failureReason` display on failed cards
- Catch-Up Summary in Command Center ⭐ NEW

**Phase 2b — Advanced Interactions:**
- Cross-column DnD with transition matrix + confirmation guards
- Quick-add task from Kanban toolbar
- "Request Rework" on Done cards
- "Retry All" batch on Failed column
- Cascading block detection

**Phase 3 — Polish & Scale:**
- System-wide Attention Bar (app shell) ⭐ NEW
- Monitoring Level / Trust Dial setting ⭐ NEW
- 3-lane compact view
- Mobile responsive layout
- Virtual scrolling for 50+ tasks
- Sprint summary modal
- Swimlanes by team
- Keyboard shortcuts (J/K, M, Enter)
- Velocity indicator

---

*Final unified design spec — 2026-03-08. Incorporates input from Designer @8baab941, PM @4b5c4761, Radical Thinker @1390cfd8, and Lead @49cbf6e1.*
