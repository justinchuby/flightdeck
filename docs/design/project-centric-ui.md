...
- **Health indicator** — green/amber/red based on agent status aggregation
...
- `/settings/advanced` — Database browser
...
- `GET /api/agents` (filtered by project)
...
| GET | `/api/dashboard/team-status` | Aggregate team health/counts | `agentRoster` + agent status aggregation |
...
The project-centric UI maps directly to the `(projectId, teamId)` scoping from the multi-team model:
...
**Project switching** maps to: user navigates to a different project → orchestrator updates the active `(projectId, teamId)` scope → events are filtered for that scope.
